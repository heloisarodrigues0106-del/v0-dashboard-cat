...
async function run() {
    const { data } = await supabase.from('tb_pedidos_inicial').select('*').limit(1);
    console.log("tb_pedidos_inicial:", Object.keys(data[0]));
    
    const { data: dataS } = await supabase.from('tb_pedidos_sentenca').select('*').limit(1);
    console.log("tb_pedidos_sentenca:", Object.keys(dataS[0]));
}
run();
